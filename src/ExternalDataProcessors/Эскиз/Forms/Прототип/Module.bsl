...
Процедура НарисоватьЭлементУправленияНаСервере(Тип, СвойстваЭлемента, Связать, Перемещение)
	...
	ИначеЕсли Тип = Тип("ГруппаФормы") Тогда
		...
		ИначеЕсли НовыйЭлемент.Вид = ВидГруппыФормы.ГруппаКнопок Тогда
			НовыйЭлемент.Отображение = ОтображениеГруппыКнопок[СвойстваЭлемента.Отображение];
			
		КонецЕсли;
		
		НовыйЭлемент.Заголовок = СвойстваЭлемента.Заголовок;
	
		НовыйЭлемент.ВертикальноеПоложениеПодчиненных = ВертикальноеПоложениеЭлемента[СвойстваЭлемента.ВертикальноеПоложениеПодчиненных];
		НовыйЭлемент.ГоризонтальноеПоложениеПодчиненных = ГоризонтальноеПоложениеЭлемента[СвойстваЭлемента.ГоризонтальноеПоложениеПодчиненных];
		
		НовыйЭлемент.ВертикальноеПоложениеВГруппе = ВертикальноеПоложениеЭлемента[СвойстваЭлемента.ВертикальноеПоложениеВГруппе];
		НовыйЭлемент.ГоризонтальноеПоложениеВГруппе = ГоризонтальноеПоложениеЭлемента[СвойстваЭлемента.ГоризонтальноеПоложениеВГруппе];
