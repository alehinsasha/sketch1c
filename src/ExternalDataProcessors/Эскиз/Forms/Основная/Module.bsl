		...
		Если ЗагрузитьПрототипНаСервере(Адрес, Идентификатор, Замена) Тогда
			...
			Для Каждого Строка Из Строки Цикл
				Если Строка.Идентификатор = Идентификатор Тогда
					...
					Если Замена Тогда
						МенеджерПроекта.УдалитьФорму(Строка.Идентификатор);
					КонецЕсли;
...
Процедура ЗагрузитьПрототипРекурсивно(Строки)
	...
	Для Каждого Строка Из Строки Цикл
		Если Строка.Тип = МенеджерПроекта.Типы().ФормаКлиентскогоПриложения Тогда
			МенеджерПроекта.ОткрытьПрототип(Строка, ЭтаФорма, Ложь);
		Иначе
			Связать = Ложь;
			Если Строка.Тип = МенеджерПроекта.Типы().ПолеФормы
				Или Строка.Тип = МенеджерПроекта.Типы().КнопкаФормы 
				Или Строка.Тип = МенеджерПроекта.Типы().ТаблицаФормы Тогда
			...
			МенеджерПроекта.ДобавитьЭлементУправления(СпециальныеСвойства, ТиповыеСвойства, Связать, Ложь);
		КонецЕсли;
...
Процедура АктивизироватьТекущийЭлемент() Экспорт
	...
	Иначе
		...
		МенеджерПроекта.АктивизироватьТекущийЭлемент(СтрокаПрототипа.Идентификатор, ТекущиеДанные.Имя);
	КонецЕсли;
...
Процедура ОпределитьГруппуКоманд()
	...
	Иначе
		...
		Если ТекущиеДанные.Тип = МенеджерПроекта.Типы().ГруппаФормы Тогда
			...
			ИначеЕсли ТекущиеДанные.Вид = МенеджерПроекта.Виды().КоманднаяПанель
		...
		ИначеЕсли ТекущиеДанные.Тип = МенеджерПроекта.Типы().КнопкаФормы Тогда
			...
			Если Родитель.Вид = МенеджерПроекта.Виды().КоманднаяПанель
				Или Родитель.Вид = МенеджерПроекта.Виды().Подменю
				Или Родитель.Вид = МенеджерПроекта.Виды().ГруппаКнопок
				Или Родитель.Вид = МенеджерПроекта.Виды().КонтекстноеМеню Тогда
				
				Элементы.ГруппаКоманды.ТекущаяСтраница = Элементы.ГруппаКоманды_КоманднаяПанель;
			КонецЕсли;
		ИначеЕсли ТекущиеДанные.Тип = МенеджерПроекта.Типы().ПолеФормы Тогда
			Родитель = ТекущиеДанные.ПолучитьРодителя();
			Если Родитель.Тип = МенеджерПроекта.Типы().ТаблицаФормы
				Или Родитель.Вид = МенеджерПроекта.Виды().ГруппаКолонок Тогда
...
Процедура ОбработатьИзменениеСвойства()
	...
	Если ТекущиеДанныеПрототип.Тип = МенеджерПроекта.Типы().ФормаКлиентскогоПриложения Тогда
		МенеджерПроекта.ИзменитьСвойство(ТекущиеДанныеПрототип.Идентификатор, ТекущиеДанныеСвойств.Свойство, ТекущиеДанныеСвойств.Значение);
	Иначе
		...
		Если ТекущиеДанныеСвойств.Свойство = "ТипЗначения" 
			...
			МенеджерПроекта.СоздатьИзменитьРеквизит(Прототип.Идентификатор, СпециальныеСвойства);
		Иначе
			МенеджерПроекта.ИзменитьСвойство(Прототип.Идентификатор, ТекущиеДанныеСвойств.Свойство, ТекущиеДанныеСвойств.Значение, Ложь, ТекущиеДанныеПрототип.Имя);
...
Процедура ВыполнитьУдалениеФормыИлиЭлементаУправления()
	...
	Если ТекущиеДанные.Тип = МенеджерПроекта.Типы().ФормаКлиентскогоПриложения Тогда
		МенеджерПроекта.УдалитьФорму(ТекущиеДанные.Идентификатор);
	...
	Иначе
		...
		Если ТекущиеДанные.Тип = МенеджерПроекта.Типы().КнопкаФормы Тогда
			МенеджерПроекта.УдалитьЭлементУправления(СтрокаПрототипа.Идентификатор, ТекущиеДанные.Имя, ТекущиеДанные.Имя, Истина);
		ИначеЕсли ТекущиеДанные.Тип = МенеджерПроекта.Типы().ПолеФормы Тогда
			МенеджерПроекта.УдалитьЭлементУправления(СтрокаПрототипа.Идентификатор, ТекущиеДанные.Имя, ТекущиеДанные.Имя, Истина,
			ТекущиеДанные.ЭтоКолонка, ТекущиеДанные.ИмяТаблицы);
		ИначеЕсли ТекущиеДанные.Тип = МенеджерПроекта.Типы().ГруппаФормы
			Или ТекущиеДанные.Тип = МенеджерПроекта.Типы().ДекорацияФормы Тогда
			
			МенеджерПроекта.УдалитьЭлементУправления(СтрокаПрототипа.Идентификатор, ТекущиеДанные.Имя, "", Ложь);
		Иначе
			МенеджерПроекта.УдалитьЭлементУправления(СтрокаПрототипа.Идентификатор, ТекущиеДанные.Имя, ТекущиеДанные.Имя, Ложь);
		КонецЕсли;
...
Процедура ВыборЦветаЗавершение(Цвет, ДополнительныеПараметры) Экспорт
	...
	ИначеЕсли Цвет.Вид = ВидЦвета.WebЦвет Тогда
		ДанныеВебЦвета = МенеджерПроекта.ПолучитьДанныеВебЦвета(Цвет);
	...
	ИначеЕсли Цвет.Вид = ВидЦвета.ЭлементСтиля Тогда
		ДанныеЦветаСтиля = МенеджерПроекта.ПолучитьДанныеЦветаСтиля(Цвет);
