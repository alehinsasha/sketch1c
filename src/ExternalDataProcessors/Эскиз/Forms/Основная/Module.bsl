...
Процедура ДобавитьЭлементУправления(Тип, Вид = "", ТипЗначения = "", ЭтоКолонка = Ложь)
	...
	Если Тип = МенеджерПроекта.Типы().ГруппаФормы Тогда
		...
		Если Вид = МенеджерПроекта.Виды().ОбычнаяГруппа Тогда
			...
			ЗаполнитьСвойствоЗначениемПоУмолчанию(НоваяСтрока, "Группировка");
			ЗаполнитьСвойствоЗначениемПоУмолчанию(НоваяСтрока, "ОтображатьЗаголовок");
...
&НаКлиенте
Процедура ЗаполнитьСвойствоЗначениемПоУмолчанию(НоваяСтрока, ИмяСвойства)
	ПараметрыОтбора = Новый Структура("Свойство", ИмяСвойства);
	ИскомыеСтроки = СвойстваФормИЭлементовУправления.НайтиСтроки(ПараметрыОтбора);
	
	Если НоваяСтрока.Тип = "ГруппаКолонок" Тогда	
		НоваяСтрока[ИмяСвойства] = ИскомыеСтроки[0].ДанныеВыбора_ГруппаКолонок[0].Значение;
		
	Иначе
		НоваяСтрока[ИмяСвойства] = ИскомыеСтроки[0].ДанныеВыбора[0].Значение;
