...
Процедура ДобавитьФорму(Команда)
	...
	ЗаполнитьСвойстваЗначениямиПоУмолчанию(НоваяСтрока,
		"ГоризонтальноеПоложениеПодчиненных,ВертикальноеПоложениеПодчиненных");
...
Процедура ОбработатьИзменениеСвойства()
	...
	Иначе
		...
		Если ТекущиеДанныеСвойств.Свойство = "ТипЗначения" Тогда
			...
			МенеджерПроекта.ИзменитьТипЗначения(Прототип.Имя, ТекущиеДанныеСвойств.Свойство, ТекущиеДанныеСвойств.Значение,
				Ложь, ТекущиеДанныеПрототип.Имя, ТекущиеДанныеПрототип.Путь);

		ИначеЕсли ТекущиеДанныеСвойств.Свойство = "СоставДаты" Тогда
			МенеджерПроекта.ИзменитьТипЗначения(Прототип.Имя, "ТипЗначения", "Дата", Ложь, ТекущиеДанныеПрототип.Имя,
				ТекущиеДанныеПрототип.Путь, ТекущиеДанныеСвойств.Значение);

		Иначе
			МенеджерПроекта.ИзменитьСвойство(Прототип.Имя, ТекущиеДанныеСвойств.Свойство, ТекущиеДанныеСвойств.Значение,
				Ложь, ТекущиеДанныеПрототип.Имя);
...
Процедура ДобавитьЭлементУправления(Тип, Вид = "", ТипЗначения = "", ЭтоКолонка = Ложь)
	...
	Если ТекущиеДанные = Неопределено Тогда
		Возврат;
	ИначеЕсли Ложь Или ТекущиеДанные.Тип = МенеджерПроекта.Типы().ФормаКлиентскогоПриложения Или ТекущиеДанные.Тип
		= МенеджерПроекта.Типы().ГруппаФормы Или ТекущиеДанные.Тип = МенеджерПроекта.Типы().ТаблицаФормы Тогда
	...
	Если Тип = МенеджерПроекта.Типы().ГруппаФормы Тогда
		...
		Если Вид = МенеджерПроекта.Виды().ОбычнаяГруппа Тогда
			...
			НоваяСтрока.Заголовок = НовоеИмя;
			ЗаполнитьСвойстваЗначениямиПоУмолчанию(НоваяСтрока,
				"Группировка,ОтображатьЗаголовок,ВертикальноеПоложениеВГруппе,ГоризонтальноеПоложениеВГруппе,ГоризонтальноеПоложениеПодчиненных,ВертикальноеПоложениеПодчиненных");
		...
		ИначеЕсли Вид = МенеджерПроекта.Виды().Страницы Тогда
			...
			НоваяСтрока.Заголовок = НовоеИмя;
			ЗаполнитьСвойстваЗначениямиПоУмолчанию(НоваяСтрока,
				"ВертикальноеПоложениеВГруппе,ГоризонтальноеПоложениеВГруппе,ВертикальноеПоложениеПодчиненных,ГоризонтальноеПоложениеПодчиненных");
		...
		ИначеЕсли Вид = МенеджерПроекта.Виды().Страница Тогда
			...
			НоваяСтрока.Заголовок = НовоеИмя;
			ЗаполнитьСвойстваЗначениямиПоУмолчанию(НоваяСтрока,
				"Группировка,ВертикальноеПоложениеВГруппе,ГоризонтальноеПоложениеВГруппе,ВертикальноеПоложениеПодчиненных,ГоризонтальноеПоложениеПодчиненных");
		...
		ИначеЕсли Вид = МенеджерПроекта.Виды().ГруппаКнопок Тогда
			...
			НоваяСтрока.Заголовок = НовоеИмя;
			ЗаполнитьСвойстваЗначениямиПоУмолчанию(НоваяСтрока,
				"Отображение,ВертикальноеПоложениеВГруппе,ГоризонтальноеПоложениеВГруппе");
		...
		ИначеЕсли Вид = МенеджерПроекта.Виды().ГруппаКолонок Тогда
			...
			НоваяСтрока.Заголовок = НовоеИмя;
			ЗаполнитьСвойстваЗначениямиПоУмолчанию(НоваяСтрока,
				"Группировка,ВертикальноеПоложениеВГруппе,ГоризонтальноеПоложениеВГруппе");
	...
	ИначеЕсли Тип = МенеджерПроекта.Типы().ПолеФормы Тогда
		...
		Если Вид = МенеджерПроекта.Виды().ПолеВвода Тогда
			Если НоваяСтрока.ЭтоКолонка Тогда
				...
				ЗаполнитьСвойстваЗначениямиПоУмолчанию(НоваяСтрока,
					"ТипЗначения,СоставДаты,ПоложениеЗаголовка,ВертикальноеПоложениеВГруппе,ГоризонтальноеПоложениеВГруппе");
		...
		ИначеЕсли Вид = МенеджерПроекта.Виды().ПолеФорматированногоДокумента Тогда
			...
			ЗаполнитьСвойстваЗначениямиПоУмолчанию(НоваяСтрока,
				"ПоложениеЗаголовка,ВертикальноеПоложениеВГруппе,ГоризонтальноеПоложениеВГруппе");
		...
		ИначеЕсли Вид = МенеджерПроекта.Виды().ПолеПереключателя Тогда
			...
			ЗаполнитьСвойстваЗначениямиПоУмолчанию(НоваяСтрока,
				"ПоложениеЗаголовка,ВидПереключателя,ВертикальноеПоложениеВГруппе,ГоризонтальноеПоложениеВГруппе");
		...
		ИначеЕсли Вид = МенеджерПроекта.Виды().ПолеКартинки Тогда
			...
			НоваяСтрока.ТипЗначения = "Строка";
			ЗаполнитьСвойстваЗначениямиПоУмолчанию(НоваяСтрока,
				"ПоложениеЗаголовка,Гиперссылка,ВертикальноеПоложениеВГруппе,ГоризонтальноеПоложениеВГруппе");
	...
	ИначеЕсли Тип = МенеджерПроекта.Типы().КнопкаФормы Тогда
		...
		Если Вид = МенеджерПроекта.Виды().ОбычнаяКнопка Тогда
			...
			НоваяСтрока.Заголовок = НовоеИмя;
			ЗаполнитьСвойстваЗначениямиПоУмолчанию(НоваяСтрока,
				"КнопкаПоУмолчанию,ВертикальноеПоложениеВГруппе,ГоризонтальноеПоложениеВГруппе");
...
Процедура РедактированиеДанныхВыбора(ИмяКолонкиДанынхВыбора)
	...
	ОткрытьФорму("ВнешняяОбработка.Эскиз.Форма.ДанныеВыбораСвойства", ПараметрыФормы, ЭтаФорма, , , ,
		ОписаниеОповещенияОЗакрытии, РежимОткрытияОкнаФормы.БлокироватьОкноВладельца);
...
Процедура ЗначениеНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	Иначе
		Если ТекущиеДанныеСвойства.ДанныеВыбора.Количество()
			Или ТекущиеДанныеСвойства.ДанныеВыбора_ГруппаКолонок.Количество() Тогда
			...
			Если ТекущиеДанныеПрототип.Вид = МенеджерПроекта.Виды().ГруппаКолонок Тогда
				...
				ПоказатьВыборИзСписка(ОписаниеОповещения, ТекущиеДанныеСвойства.ДанныеВыбора_ГруппаКолонок,
			...
			Иначе
				...
				Иначе
					ОписаниеОповещения = Новый ОписаниеОповещения("ВыборИзВыпадающегоСпискаЗавершение", ЭтотОбъект);
					ПоказатьВыборИзСписка(ОписаниеОповещения, ТекущиеДанныеСвойства.ДанныеВыбора,
						Элементы.СвойстваФормИЭлементовУправления.ТекущийЭлемент);
		...
		Иначе
			...
			Если ТипЗнч(ТекущиеДанныеСвойства.Значение) = Тип("Строка") Тогда
				ПоказатьВводСтроки(ОписаниеОповещения, ТекущиеДанныеСвойства.Значение,
					ТекущиеДанныеСвойства.СвойствоПредставление, , Истина);

			ИначеЕсли ТипЗнч(ТекущиеДанныеСвойства.Значение) = Тип("Число") Тогда
				ПоказатьВводЧисла(ОписаниеОповещения, ТекущиеДанныеСвойства.Значение,
					ТекущиеДанныеСвойства.СвойствоПредставление);
