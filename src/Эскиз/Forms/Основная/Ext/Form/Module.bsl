...
Процедура ОпределитьСвойства(ТекущиеДанные) Экспорт
	...
	Для Каждого Строка Из ИскомыеСтроки Цикл
		...
		Если ЕстьДанныеВыбора(Строка) Тогда
			ИскомоеЗначение = ТекущееЗначение;
		...
		Иначе
			Строка.ЗначениеПредставление = ?(ЗначениеЗаполнено(ТекущееЗначение), ТекущееЗначение, "...");
...
Процедура ОбработатьИзменениеСвойства()
	...
	//удалить// Если ТекущиеДанныеСвойств.Свойство = "Заголовок" Или ТекущиеДанныеСвойств.Свойство = "КоличествоКолонок" Тогда
	...
	Иначе
		...
		Если ТекущиеДанныеСвойств.Свойство = "ТипЗначения" Тогда
			//удалить// ТекущиеДанныеПрототип.СписокВыбора = Новый СписокЗначений;
...
Функция ЗаполнитьСвойстваЗначениямиПоУмолчанию(НоваяСтрока)
	// Только свойства с заполненными данными выбора
	КлючиТиповыхСвойств = "";
	...
	Если НоваяСтрока.Вид = МенеджерПроекта.Виды().ПолеВвода Тогда
		ТекущееСвойство = "СоставДаты";
		ПараметрыОтбора = Новый Структура("Свойство", ТекущееСвойство);
		ИскомыеСтроки = СвойстваФормИЭлементовУправления.НайтиСтроки(ПараметрыОтбора);
		НоваяСтрока[ТекущееСвойство] = ИскомыеСтроки[0].ДанныеВыбора[0].Значение;
