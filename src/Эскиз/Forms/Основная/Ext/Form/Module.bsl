...
Процедура ДобавитьФорму(Команда)
	...
	ЗаполнитьСвойстваЗначениямиПоУмолчанию(НоваяСтрока);
...
Процедура ОпределитьСвойства(ТекущиеДанные) Экспорт
	...
	Для Каждого Строка Из ИскомыеСтроки Цикл
		...
		Если Истина
			И ПустаяСтрока(ТекущееЗначение)
			И Строка.Свойство <> "СписокВыбора"
			И Строка.Свойство <> "Картинка"
			Тогда
		...
		ИначеЕсли Ложь
			Или ТекущиеДанные.Вид = МенеджерПроекта.Виды().ОбычнаяКнопка
			Или ТекущиеДанные.Вид = МенеджерПроекта.Виды().КнопкаКоманднойПанели
			Или ТекущиеДанные.Вид = МенеджерПроекта.Виды().Гиперссылка
			Или ТекущиеДанные.Вид = МенеджерПроекта.Виды().ГиперссылкаКоманднойПанели
			Тогда
			
			ТекущиеДанныеВыбора = Строка.ДанныеВыбора_Кнопка;
...
Процедура ОбработатьИзменениеСвойства()
	...
	Если Ложь Или ТекущиеДанныеСвойств.Свойство = "Заголовок" Или ТекущиеДанныеСвойств.Свойство = "КоличествоКолонок" Тогда
	...
	Если ТекущиеДанныеПрототип.Тип = МенеджерПроекта.Типы().ФормаКлиентскогоПриложения Тогда
		МенеджерПроекта.ИзменитьСвойство(ТекущиеДанныеПрототип.Имя, ТекущиеДанныеСвойств.Свойство,
	...
	Иначе
		...
		Иначе
			...
			Если Истина И ТекущиеДанныеСвойств.Свойство = "КнопкаПоУмолчанию" И ТекущиеДанныеСвойств.Значение = "Да" Тогда
...
Процедура ДобавитьЭлементУправления(Тип, Вид = "", ТипЗначения = "", ЭтоКолонка = Ложь)
	...
	КлючиСпециальныеСвойства = "Имя,Тип,Вид,Родитель,Прототип,ТипЗначения,Путь,ПутьКДанным";
	СпециальныеСвойства = Новый Структура(КлючиСпециальныеСвойства);
	ЗаполнитьЗначенияСвойств(СпециальныеСвойства, НоваяСтрока);
	
	КлючиТиповыхСвойств = ЗаполнитьСвойстваЗначениямиПоУмолчанию(НоваяСтрока);
	ИсключитьСвойстваРеквизитаФормы(КлючиТиповыхСвойств);
...
&НаКлиенте
Процедура ИсключитьСвойстваРеквизитаФормы(КлючиТиповыхСвойств) 
	КлючиТиповыхСвойств = СтрЗаменить(КлючиТиповыхСвойств, "ТипЗначения,", "");
	КлючиТиповыхСвойств = СтрЗаменить(КлючиТиповыхСвойств, "Длина,", "");
	КлючиТиповыхСвойств = СтрЗаменить(КлючиТиповыхСвойств, "Точность,", "");
	КлючиТиповыхСвойств = СтрЗаменить(КлючиТиповыхСвойств, "СоставДаты,", "");
...
Функция СохранитьСвойстваВФайлНаСервере()
	...
	Результат.ПутьКФайлу = СтрЗаменить(РеквизитФормыВЗначение("Объект").ИспользуемоеИмяФайла, "Эскиз.epf",
	"out\Свойства.json");
...
Процедура ВыполнитьПеремещение(ПеремещаемыеЭлементы, ИдентификаторСтрокиПриемника, Перемещенные,
	...
	ЭтоГруппа = Ложь Или СтрокаПриемник.Тип = МенеджерПроекта.Типы().ФормаКлиентскогоПриложения Или СтрокаПриемник.Тип
	= МенеджерПроекта.Типы().ГруппаФормы Или СтрокаПриемник.Тип = МенеджерПроекта.Типы().ТаблицаФормы;
...
&НаКлиенте
Функция ЗаполнитьСвойстваЗначениямиПоУмолчанию(НоваяСтрока) // Только свойства с заполненными данными выбора
	КлючиТиповыхСвойств = "";
	...
	ИскомыеСтроки = СвойстваФормИЭлементовУправления.НайтиСтроки(ПараметрыОтбора);
	Для Каждого Строка Из ИскомыеСтроки Цикл
		КлючиТиповыхСвойств = КлючиТиповыхСвойств + "," + Строка.Свойство;
		
		Если ЕстьДанныеВыбора(Строка) Тогда
			Если НоваяСтрока.Вид = МенеджерПроекта.Виды().ГруппаКолонок Тогда
				Если Строка.Свойство = "Группировка" Тогда
					НоваяСтрока[Строка.Свойство] = Строка.ДанныеВыбора_ГруппаКолонок[0].Значение;
					
				Иначе
					НоваяСтрока[Строка.Свойство] = Строка.ДанныеВыбора[0].Значение;
					
				КонецЕсли;
				
			ИначеЕсли Ложь
				Или НоваяСтрока.Вид = МенеджерПроекта.Виды().ОбычнаяКнопка
				Или НоваяСтрока.Вид = МенеджерПроекта.Виды().КнопкаКоманднойПанели
				Или НоваяСтрока.Вид = МенеджерПроекта.Виды().Гиперссылка
				Или НоваяСтрока.Вид = МенеджерПроекта.Виды().ГиперссылкаКоманднойПанели
				Тогда
				
				Если Строка.Свойство = "Отображение" Тогда
					НоваяСтрока[Строка.Свойство] = Строка.ДанныеВыбора_Кнопка[0].Значение;
	...
	Возврат Сред(КлючиТиповыхСвойств, 2);
КонецФункции
...
&НаКлиенте
Функция ЕстьДанныеВыбора(Строка) 
	Результат = Ложь;
	
	ИменаДанныхВыбора = ПолучитьИменаДанныхВыбора();
	Для Каждого ИмяДанныехВыбора Из ИменаДанныхВыбора Цикл
		Если Строка[ИмяДанныехВыбора].Количество() Тогда
			Результат = Истина;
...
Процедура ЗначениеНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	Если ТипЗнч(ТекущиеДанныеСвойства.Значение) = Тип("СписокЗначений") Тогда
		...
		ОткрытьФорму("ВнешняяОбработка.Эскиз.Форма.СписокВыбораПоля", ПараметрыФормы, ЭтотОбъект, , , ,
		ОписаниеОповещенияОЗакрытии, РежимОткрытияОкнаФормы.БлокироватьОкноВладельца);
		
	Иначе
		Если ЕстьДанныеВыбора(ТекущиеДанныеСвойства) Тогда
			...
			ИначеЕсли Ложь
				Или ТекущиеДанныеПрототип.Вид = МенеджерПроекта.Виды().ОбычнаяКнопка
				Или ТекущиеДанныеПрототип.Вид = МенеджерПроекта.Виды().КнопкаКоманднойПанели
				Или ТекущиеДанныеПрототип.Вид = МенеджерПроекта.Виды().Гиперссылка
				Или ТекущиеДанныеПрототип.Вид = МенеджерПроекта.Виды().ГиперссылкаКоманднойПанели
				Тогда
				
				ПоказатьВыборИзСписка(ОписаниеОповещения, ТекущиеДанныеСвойства.ДанныеВыбора_Кнопка, Элементы.СвойстваФормИЭлементовУправления.ТекущийЭлемент);
			...
			КонецЕсли;
		Иначе
			// нужно дать ввести значение (заголовок, количество колонок и т.д.) или выбрать катинку
