...
Процедура ФормыИЭлементыУправленияИзМассиваРекурсивно(Строки, Родитель)
	...
	Элементы.ФормыИЭлементыУправления.Развернуть(ФормыИЭлементыУправления.ПолучитьЭлементы()[0].ПолучитьИдентификатор(),
	Истина);
...
Процедура РасположениеПроектаОчистка(Элемент, СтандартнаяОбработка)
	...
	ПоказатьВопрос(ОписаниеОповещения, "Эскиз будет приведен в исходное состояние! Продолжить?",
...
Функция ПолучитьПараметрыОтбораСвойств(ТекущиеДанные)
	...
	ИначеЕсли Истина И ТекущиеДанные.Тип = МенеджерПроекта.Типы().ПолеФормы И ТекущиеДанные.Вид
		= МенеджерПроекта.Виды().ПолеВвода Тогда
...
Процедура ОпределитьСвойства(ТекущиеДанные) Экспорт
	...
	Для Каждого Строка Из ИскомыеСтроки Цикл
		...
		Если Строка.ВариантЗаполнения = МенеджерПроекта.ВариантыЗаполнения().ВыборИзВыпадающегоСписка Тогда
...
Процедура ОпределитьГруппуКоманд(ТекущиеДанные)
	...
	ИначеЕсли ТекущиеДанные.Тип = МенеджерПроекта.Типы().ГруппаФормы Тогда
		...
		ИначеЕсли Ложь Или ТекущиеДанные.Вид = МенеджерПроекта.Виды().КоманднаяПанель Или ТекущиеДанные.Вид
			= МенеджерПроекта.Виды().Подменю Или ТекущиеДанные.Вид = МенеджерПроекта.Виды().ГруппаКнопок
			Или ТекущиеДанные.Вид = МенеджерПроекта.Виды().КонтекстноеМеню Тогда
	...
	ИначеЕсли ТекущиеДанные.Тип = МенеджерПроекта.Типы().КнопкаФормы Тогда
		...
		Если Ложь Или Родитель.Вид = МенеджерПроекта.Виды().КоманднаяПанель Или Родитель.Вид = МенеджерПроекта.Виды().Подменю
			Или Родитель.Вид = МенеджерПроекта.Виды().ГруппаКнопок Или Родитель.Вид = МенеджерПроекта.Виды().КонтекстноеМеню Тогда
	...
	ИначеЕсли ТекущиеДанные.Тип = МенеджерПроекта.Типы().ПолеФормы Тогда
		Родитель = ТекущиеДанные.ПолучитьРодителя();
		Если Ложь Или Родитель.Тип = МенеджерПроекта.Типы().ТаблицаФормы Или Родитель.Вид
			= МенеджерПроекта.Виды().ГруппаКолонок Тогда
...
Процедура ОбработатьИзменениеСвойства()
	...
	Иначе
		...
		Если ТекущиеДанныеСвойств.Свойство = "ТипЗначения" 
			...
			МенеджерПроекта.СоздатьИзменитьРеквизит(Прототип.Имя, СпециальныеСвойства);
...
Процедура ДобавитьЭлементУправления(Тип, Вид = "", ТипЗначения = "", ЭтоКолонка = Ложь)
	...
	Если ТекущиеДанные = Неопределено Тогда
		Возврат;
	ИначеЕсли ТекущиеДанные.Тип = МенеджерПроекта.Типы().ФормаКлиентскогоПриложения
		Или ТекущиеДанные.Тип = МенеджерПроекта.Типы().ГруппаФормы
		Или ТекущиеДанные.Тип = МенеджерПроекта.Типы().ТаблицаФормы Тогда
	...
	МенеджерПроекта.ДобавитьЭлементУправления(СпециальныеСвойства, ТиповыеСвойства, Связать, Перемещение);
...
Процедура ОпределитьВидимостьПодсказокКоманд(ЭтоИзменение = Истина)
	...
	Для Каждого Страница Из Страницы Цикл
		...
		Для Каждого ЭлементСтраницы Из ЭлементыСтраницы Цикл
			Если Истина И ТипЗнч(ЭлементСтраницы) = Тип("КнопкаФормы") И ЭлементСтраницы.ИмяКоманды
				<> "ВидимостьПодсказокКоманд" Тогда
...
Функция ИерархияНеКорректна(СтрокаПриемник, СтрокаИсточник)
	...
	Если СтрокаПриемник.Тип = МенеджерПроекта.Типы().ГруппаФормы И СтрокаПриемник.Вид = МенеджерПроекта.Виды().Страницы Тогда
		...
		КонецЕсли;
	ИначеЕсли СтрокаПриемник.Тип = МенеджерПроекта.Типы().ГруппаФормы И СтрокаПриемник.Вид
		= МенеджерПроекта.Виды().КоманднаяПанель Тогда
		Если СтрокаИсточник.Тип = МенеджерПроекта.Типы().ГруппаФормы И СтрокаИсточник.Вид = МенеджерПроекта.Виды().Подменю Тогда
			Результат = Ложь;
			
		ИначеЕсли СтрокаИсточник.Тип = МенеджерПроекта.Типы().ГруппаФормы И СтрокаИсточник.Вид
			= МенеджерПроекта.Виды().ГруппаКнопок Тогда
			Результат = Ложь;
			
		ИначеЕсли СтрокаИсточник.Тип = МенеджерПроекта.Типы().КнопкаФормы И СтрокаИсточник.Вид
			= МенеджерПроекта.Виды().КнопкаКоманднойПанели Тогда
			Результат = Ложь;
			
		ИначеЕсли СтрокаИсточник.Тип = МенеджерПроекта.Типы().КнопкаФормы И СтрокаИсточник.Вид
			= МенеджерПроекта.Виды().ГиперссылкаКоманднойПанели Тогда
			Результат = Ложь;
	...
	ИначеЕсли СтрокаПриемник.Тип = МенеджерПроекта.Типы().ТаблицаФормы Тогда
		...
		ИначеЕсли СтрокаИсточник.Тип = МенеджерПроекта.Типы().ГруппаФормы И СтрокаИсточник.Вид
			= МенеджерПроекта.Виды().КонтекстноеМеню Тогда
			Результат = Ложь;
...
Процедура ВыполнитьУдалениеФормыИлиЭлементаУправления()
	...
	Иначе
		...
		ИначеЕсли Ложь Или ТекущиеДанные.Тип = МенеджерПроекта.Типы().ГруппаФормы Или ТекущиеДанные.Тип
			= МенеджерПроекта.Типы().ДекорацияФормы Тогда
...
Процедура ЗаполнитьСвойстваПервымЗначениемДанныхВыбора(НоваяСтрока)
	...
	ПараметрыОтбора.Вставить("ВариантЗаполнения", МенеджерПроекта.ВариантыЗаполнения().ВыборИзВыпадающегоСписка);
...
Функция ПолучитьКлючиСпециальныеСвойства()
	Результат = "Имя,Тип,Вид,Родитель,Прототип,Путь,ПутьКДанным,ТипЗначения,Длина,Точность,СоставДаты";
...
Процедура ЗначениеНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	Если ТекущиеДанныеСвойства.ВариантЗаполнения = МенеджерПроекта.ВариантыЗаполнения().ВводМножестваЗначений Тогда
		ПараметрыФормы = Новый Структура;
	...
	ИначеЕсли ТекущиеДанныеСвойства.ВариантЗаполнения = МенеджерПроекта.ВариантыЗаполнения().ВыборИзВыпадающегоСписка Тогда
		...
		ПоказатьВыборИзСписка(ОписаниеОповещения, ТекущиеДанныеСвойства[ИмяДанныхВыбора], Элементы.СвойстваФормИЭлементовУправления.ТекущийЭлемент);
		
	ИначеЕсли ТекущиеДанныеСвойства.ВариантЗаполнения = МенеджерПроекта.ВариантыЗаполнения().ВыборКартинки Тогда
	...
	ИначеЕсли ТекущиеДанныеСвойства.ВариантЗаполнения = МенеджерПроекта.ВариантыЗаполнения().ВводЗначения Тогда
		ПараметрыФормы = Новый Структура;
		ПараметрыФормы.Вставить("Заголовок", ТекущиеДанныеСвойства.СвойствоПредставление);
		ПараметрыФормы.Вставить("Значение", ТекущиеДанныеСвойства.Значение);
		...
		КонецЕсли;
	ИначеЕсли ТекущиеДанныеСвойства.ВариантЗаполнения = МенеджерПроекта.ВариантыЗаполнения().ВыборЦвета Тогда
	...
	ИначеЕсли ТекущиеДанныеСвойства.ВариантЗаполнения = МенеджерПроекта.ВариантыЗаполнения().ВыборШрифта Тогда
...
Функция ПолучитьИмяДанныхВыбора(Свойство, Тип, Вид) 
	...
	Если Свойство = "Группировка" Тогда
		Если Вид = МенеджерПроекта.Виды().ГруппаКолонок Тогда
	...
	ИначеЕсли Свойство = "Отображение" Тогда
		Если Ложь
			Или Тип = МенеджерПроекта.Типы().КнопкаФормы
			Или Вид = МенеджерПроекта.Виды().Подменю
			Тогда
...
Процедура ВыборИзВыпадающегоСпискаЗавершение(ВыбранныйЭлемент, ДополнительныеПараметры) Экспорт
	...
	Если Не ВыбранныйЭлемент = Неопределено Тогда
		...
		ТекущиеДанныеПрототип = Элементы.ФормыИЭлементыУправления.ТекущиеДанные;
		ОпределитьСвойства(ТекущиеДанныеПрототип);
...
Процедура ВыборЦветаЗавершение(Цвет, ДополнительныеПараметры) Экспорт
	...
	Если Цвет.Вид = ВидЦвета.WebЦвет Тогда
		ТекущиеДанныеСвойства.Значение = "WebЦвет.";
		ТекущиеДанныеСвойства.ЗначениеПредставление = "Веб-цвет: " + "";
		
	ИначеЕсли Цвет.Вид = ВидЦвета.Абсолютный Тогда
		ТекущиеДанныеСвойства.Значение = "Абсолютный.";
		ТекущиеДанныеСвойства.ЗначениеПредставление = "Абсолютный: " + "";
		
	ИначеЕсли Цвет.Вид = ВидЦвета.ЭлементСтиля Тогда
		ДанныеЦветаСтиля = ПолучитьДанныеЦветаСтиля(Цвет);
...
Функция ПолучитьДанныеЦветаСтиля(Цвет) 
	Результат = Новый Структура("Значение,Представление");
	
	ВсеЦветаСтиля = Новый Соответствие;
	ВсеЦветаСтиля.Вставить(ЦветаСтиля.АльтернативныйЦветФонаПоля, "АльтернативныйЦветФонаПоля");
	ВсеЦветаСтиля.Вставить(ЦветаСтиля.ЦветАктивности, "ЦветАктивности");
	ВсеЦветаСтиля.Вставить(ЦветаСтиля.ЦветАкцента, "ЦветАкцента");
	ВсеЦветаСтиля.Вставить(ЦветаСтиля.ЦветВажного, "ЦветВажного");
	ВсеЦветаСтиля.Вставить(ЦветаСтиля.ЦветДополнительнойНавигации, "ЦветДополнительнойНавигации");
	ВсеЦветаСтиля.Вставить(ЦветаСтиля.ЦветЛинииОтчета, "ЦветЛинииОтчета");
	ВсеЦветаСтиля.Вставить(ЦветаСтиля.ЦветНавигации, "ЦветНавигации");
	ВсеЦветаСтиля.Вставить(ЦветаСтиля.ЦветОсобогоТекста, "ЦветОсобогоТекста");
	ВсеЦветаСтиля.Вставить(ЦветаСтиля.ЦветОтрицательногоЧисла, "ЦветОтрицательногоЧисла");
	ВсеЦветаСтиля.Вставить(ЦветаСтиля.ЦветРамки, "ЦветРамки");
	ВсеЦветаСтиля.Вставить(ЦветаСтиля.ЦветРамкиКнопки, "ЦветРамкиКнопки");
	ВсеЦветаСтиля.Вставить(ЦветаСтиля.ЦветТекстаВыделенияПоля, "ЦветТекстаВыделенияПоля");
	ВсеЦветаСтиля.Вставить(ЦветаСтиля.ЦветТекстаКнопки, "ЦветТекстаКнопки");
	ВсеЦветаСтиля.Вставить(ЦветаСтиля.ЦветТекстаПодвалаТаблицы, "ЦветТекстаПодвалаТаблицы");
	ВсеЦветаСтиля.Вставить(ЦветаСтиля.ЦветТекстаПодсказки, "ЦветТекстаПодсказки");
	ВсеЦветаСтиля.Вставить(ЦветаСтиля.ЦветТекстаПоля, "ЦветТекстаПоля");
	ВсеЦветаСтиля.Вставить(ЦветаСтиля.ЦветТекстаФормы, "ЦветТекстаФормы");
	ВсеЦветаСтиля.Вставить(ЦветаСтиля.ЦветТекстаШапкиТаблицы, "ЦветТекстаШапкиТаблицы");
	ВсеЦветаСтиля.Вставить(ЦветаСтиля.ЦветФонаВыделенияПоля, "ЦветФонаВыделенияПоля");
	ВсеЦветаСтиля.Вставить(ЦветаСтиля.ЦветФонаГруппировкиОтчета1, "ЦветФонаГруппировкиОтчета1");
	ВсеЦветаСтиля.Вставить(ЦветаСтиля.ЦветФонаГруппировкиОтчета2, "ЦветФонаГруппировкиОтчета2");
	ВсеЦветаСтиля.Вставить(ЦветаСтиля.ЦветФонаКнопки, "ЦветФонаКнопки");
	ВсеЦветаСтиля.Вставить(ЦветаСтиля.ЦветФонаПодвалаТаблицы, "ЦветФонаПодвалаТаблицы");
	ВсеЦветаСтиля.Вставить(ЦветаСтиля.ЦветФонаПодсказки, "ЦветФонаПодсказки");
	ВсеЦветаСтиля.Вставить(ЦветаСтиля.ЦветФонаПоля, "ЦветФонаПоля");
	ВсеЦветаСтиля.Вставить(ЦветаСтиля.ЦветФонаФормы, "ЦветФонаФормы");
	ВсеЦветаСтиля.Вставить(ЦветаСтиля.ЦветФонаШапкиОтчета, "ЦветФонаШапкиОтчета");
	ВсеЦветаСтиля.Вставить(ЦветаСтиля.ЦветФонаШапкиТаблицы, "ЦветФонаШапкиТаблицы");
