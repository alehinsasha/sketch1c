...
Процедура ФормыИЭлементыУправленияПеретаскивание(Элемент, ПараметрыПеретаскивания, СтандартнаяОбработка, Строка, Поле)
	...
	СтрокаПриемник = ФормыИЭлементыУправления.НайтиПоИдентификатору(Строка);
	ЭтоГруппа = СтрокаПриемник.ПолучитьЭлементы().Количество();
	...
	Если Ложь
		Или ЭтоГруппа
		Или ЭтоПоследнийЭлементУровня
		Тогда
		
		// Добавить в коллекцию
		
	Иначе
		// Вставить перед или после строки-приемника

	КонецЕсли;
	
	//Имена = Новый Массив;
	
	СтрокиИсточник = Новый Массив;
	Для Каждого Строка Из ПараметрыПеретаскивания.Значение Цикл
		СтрокаИсточник = ФормыИЭлементыУправления.НайтиПоИдентификатору(Строка);	
		Если СтрокиИсточник.Найти(СтрокаИсточник) = Неопределено Тогда
			СтрокиИсточник.Добавить(СтрокиИсточник);
