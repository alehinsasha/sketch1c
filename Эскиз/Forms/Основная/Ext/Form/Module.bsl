...
Процедура ОпределитьСвойства(ТекущиеДанные) Экспорт
	...
	Для Каждого Строка Из ИскомыеСтроки Цикл
		...
		Иначе
			Если ТипЗнч(ТекущееЗначение) = Тип("Строка") И ПустаяСтрока(ТекущееЗначение) И Строка.Свойство = "Картинка" Тогда
				Строка.ЗначениеПредставление = "Выберите картинку";
...
Процедура СвойстваФормИЭлементовУправленияПриНачалеРедактирования(Элемент, НоваяСтрока, Копирование)
	...
	Если ТекущиеДанные.Свойство = "Картинка" Тогда
		ДиалогВыбораФайла = Новый ДиалогВыбораФайла(РежимДиалогаВыбораФайла.Открытие);
		ДиалогВыбораФайла.МножественныйВыбор = Ложь;
	...
	ИначеЕсли ТекущиеДанные.ДанныеВыбора.Количество() Тогда
		Если ТекущиеДанные = Неопределено Тогда
			Возврат;
		КонецЕсли;
...
&НаКлиенте
Процедура ПоказатьЭскиз(Команда)
	Элементы.ГруппаРежимыЭскиза.ТекущаяСтраница = Элементы.ГруппаОбычныйРежим;
	Элементы.ГруппаОбычныйРежим.Видимость = Истина;
	
КонецПроцедуры

&НаКлиенте
Процедура СкрытьЭскиз(Команда)
	Элементы.ГруппаРежимыЭскиза.ТекущаяСтраница = Элементы.ГруппаСкрытыйРежим;
