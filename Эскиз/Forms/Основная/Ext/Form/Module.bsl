...
Процедура ВыбратьПроектЗавершение(ВыбранныеФайлы, ДополнительныеПараметры) Экспорт
	...
	//ОписаниеОповещенияОЗавершении = Новый ОписаниеОповещения("ЗагрузитьПроект", ЭтотОбъект);
	//НачатьПомещениеФайлаНаСервер(ОписаниеОповещенияОЗавершении,,,, РасположениеПроекта);
	
	Проект = ДесериализоватьИзJSON(РасположениеПроекта, Истина);
	ЗагрузитьПроектРекурсивно(Проект, ФормыИЭлементыУправления);

КонецПроцедуры

&НаКлиенте
Процедура ЗагрузитьПроектРекурсивно(Строки, Родитель) 
	Для Каждого Строка Из Строки Цикл
		...
		Иначе
			Если НоваяСтрока.Тип = МенеджерПроекта.Типы().ГруппаФормы Тогда
				Если НоваяСтрока.Вид = МенеджерПроекта.Виды().ОбычнаяГруппа Тогда
					Ключи = "Имя,Заголовок,Тип,Родитель,Прототип,ОтображатьЗаголовок";
					Связать = Ложь;
			...
			//ДобавитьЭлементУправления(НоваяСтрока.Тип, НоваяСтрока.Вид);
			
		КонецЕсли;

		ДочерниеСтроки = Строка.Строки;
		Если ДочерниеСтроки.Количество() Тогда
			ЗагрузитьПроектРекурсивно(ДочерниеСтроки, НоваяСтрока);
...
&НаКлиенте
Процедура ЗагрузитьПроект(ОписаниеПомещенногоФайла, ДополнительныеПараметры) Экспорт
	ЗагрузитьПроектНаСервере(ОписаниеПомещенногоФайла.Адрес);
	ПодключитьОбработчикОжидания("СоздатьФормыИЭлементыУправления", 0.1, Истина);

КонецПроцедуры

&НаКлиенте
Процедура СоздатьФормыИЭлементыУправления() Экспорт
	Проект = ДесериализоватьИзJSON(РасположениеПроекта, Истина);
	//ЗагрузитьПроектРекурсивно(Проект);
	
КонецПроцедуры

&НаСервере
Процедура ЗагрузитьПроектНаСервере(Адрес) 
	...
	Проект = ДесериализоватьИзJSON(ИмяВременногоФайла, Истина);
	ЗагрузитьПроектНаСервереРекурсивно(Проект, ФормыИЭлементыУправленияНаСервере);
...
&НаСервере
Функция ЗагрузитьПроектНаСервереРекурсивно(Проект, ФормыИЭлементыУправленияНаСервере)
	Для Каждого Строка Из Проект Цикл
		НоваяСтрока = ФормыИЭлементыУправленияНаСервере.Строки.Добавить();
		ЗаполнитьЗначенияСвойств(НоваяСтрока, Строка,, "Строки");
		
		ДочерниеСтроки = Строка.Строки;
		Если ДочерниеСтроки.Количество() Тогда
			ЗагрузитьПроектНаСервереРекурсивно(ДочерниеСтроки, НоваяСтрока);
...
Функция СохранитьПроектНаСервере() 
	...
	Проект = СохранитьПроектНаСервереРекурсивно(ФормыИЭлементыУправленияНаСервере.Строки, Ключи);
...
&НаСервере
Функция СохранитьПроектНаСервереРекурсивно(Строки, Ключи)
	...
	Для Каждого Строка Из Строки Цикл
		...
		ДочерниеСтроки = Строка.Строки;
		Если ДочерниеСтроки.Количество() Тогда
			ДанныеСтроки.Строки = СохранитьПроектНаСервереРекурсивно(ДочерниеСтроки, Ключи);		
...
Процедура ЗначениеПриИзменении(Элемент)
	...
	Иначе
		Прототип = ПолучитьСтрокуПрототипаРекурсивно(ТекущиеДанныеПрототип).Имя;
		Если ТекущиеДанныеСвойств.Свойство = "ТипЗначения" Тогда
			МенеджерПроекта.ИзменитьТипЗначения(Прототип, ТекущиеДанныеСвойств.Свойство, ТекущиеДанныеСвойств.Значение, Ложь, ТекущиеДанныеПрототип.Имя);
