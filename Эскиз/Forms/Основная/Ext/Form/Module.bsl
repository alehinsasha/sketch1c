...
Процедура СохранитьПроектПродолжение() 
	...
	НовыйФайл = Новый ОписаниеПередаваемогоФайла(РасположениеПроекта, Адрес);
...
&НаКлиенте
Процедура СохранитьПроектЗавершение(ПолученныеФайлы, ДополнительныеПараметры) Экспорт
	//todo
...
Процедура ДобавитьЭлементУправления(Тип, Вид = "", ТипЗначения = "", ЭтоКолонка = Ложь)
	...
	ИначеЕсли Тип = МенеджерПроекта.Типы().ПолеФормы Тогда
		...
		ИначеЕсли Вид = МенеджерПроекта.Виды().ПолеНадписи Тогда
			...
			НоваяСтрока.ПутьКДанным = НовоеИмя;
			НоваяСтрока.Заголовок = НовоеИмя;
			НоваяСтрока.ПоложениеЗаголовка = "Авто";
			НоваяСтрока.Путь = "";
		...
		ИначеЕсли Вид = МенеджерПроекта.Виды().ПолеПереключателя Тогда
			...
			НоваяСтрока.ТипЗначения = "Строка";
			НоваяСтрока.ПоложениеЗаголовка = "Авто";
			НоваяСтрока.ВидПереключателя = "Авто";
...
Процедура СохранитьСвойстваВФайл(Команда)
	...
	ПолучаемыеФайлы = Новый Массив;
	НовыйФайл = Новый ОписаниеПередаваемогоФайла(Результат.ПутьКФайлу, Результат.Адрес);
	ПолучаемыеФайлы.Добавить(НовыйФайл);
...
Процедура ВыполнитьЗагрузкуСвойствИзФайла() 
	...
	НачатьПомещениеФайла(ОписаниеОповещенияОЗавершении,, РасположениеФайлаСвойств, Ложь);
...
&НаКлиенте
Процедура ЗагрузитьСвойстваИзФайлаЗавершение(ФайлВыбран, Адрес, ВыбранноеИмяФайла, ДополнительныеПараметры) Экспорт
