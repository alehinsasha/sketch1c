...
&НаКлиенте
Процедура ВыполнитьПеремещение(ПеремещаемыеЭлементы, ИдентификаторСтрокиПриемника, Перемещенные, ПерваяИтерация = Ложь)
	...
	Для Каждого СтрокаИсточник Из ПеремещаемыеЭлементы Цикл
		...
		Если Перемещенные.Найти(СтрокаИсточник.Имя) = Неопределено Тогда
			...
			Если НовыйЭлемент <> Неопределено Тогда
				...
				НовыйЭлемент.Родитель = НовыйЭлемент.ПолучитьРодителя().Имя;
				
				Если ПрототипИсточник = НовыйЭлемент.Прототип Тогда
