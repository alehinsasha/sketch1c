...
Процедура СохранитьПроектВФайл() 
	...
	ЗаписатьJSON(Писатель, Проект, НастройкаСериализации);
	
	ХХХ = Новый ТекстовыйДокумент;
	ХХХ.УстановитьТекст(Писатель.Закрыть());

	ХХХ.НачатьЗапись(Неопределено, РасположениеПроекта);
...
Процедура ДобавитьФорму(Команда)
	НоваяФорма = ПолучитьФорму("ВнешняяОбработка.Эскиз.Форма.Прототип",, ЭтаФорма, Истина);
	НоваяФорма.ЗакрыватьПриЗакрытииВладельца = Истина;
	
	Имя = "1";
	
	Формы.Вставить(Имя, НоваяФорма);
	
	ОписаниеФормы = ПодготовитьОписаниеФормы();
	ОписаниеФормы.Имя = Имя;
	
	Проект.Добавить(ОписаниеФормы);

КонецПроцедуры


&НаКлиенте
Функция ПодготовитьОписаниеФормы() 
