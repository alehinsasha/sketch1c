...
Функция СохранитьПроектНаСервере() 
	...
	СохранитьПроектНаСервереРекурсивно(Проект, ФормыИЭлементыУправленияНаСервере.Строки, Ключи);

	Json = СериализоватьВJSON(Проект);

	Писатель = Новый ЗаписьТекста(Результат, КодировкаТекста.UTF8);
...
&НаСервере
Функция СохранитьПроектНаСервереРекурсивно(Проект, Строки, Ключи)
	...
	Для Каждого Строка Из Строки Цикл
		...
		Если ДочерниеСтроки.Количество() Тогда
			ДанныеСтроки.Строки = СохранитьПроектНаСервереРекурсивно(Проект, ДочерниеСтроки, Ключи);		
