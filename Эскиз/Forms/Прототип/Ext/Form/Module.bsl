...
Функция НарисоватьЭлементУправленияНаСервере(Тип, СвойстваЭлемента, Связать, Перемещение)
	...
	Если Тип = Тип("КнопкаФормы") Тогда
		Если Связать Тогда
			ДобавитьКоманду(СвойстваЭлемента.Имя, СвойстваЭлемента.Действие, СвойстваЭлемента.Заголовок);
...
&НаКлиенте
Процедура ПереместитьЭлементУправления(Тип, ИмяПеремещаемого, СвойстваПеремещаемого) Экспорт
	ПереместитьЭлементУправленияНаСервере(Тип, ИмяПеремещаемого, СвойстваПеремещаемого);
	ЭтаФорма.ОбновитьОтображениеДанных();
	
КонецПроцедуры

&НаСервере
Процедура ПереместитьЭлементУправленияНаСервере(Тип, ИмяПеремещаемого, СвойстваПеремещаемого)
	Если СтеретьЭлементУправленияНаСервере(ИмяПеремещаемого, ИмяПеремещаемого, Неопределено) Тогда
		НарисоватьЭлементУправленияНаСервере(Тип, СвойстваПеремещаемого,, Истина);
...
&НаКлиенте
Процедура ИзменитьСвойство(Свойство, Значение, ЭтоФорма, ИмяЭлемента) Экспорт
	ИзменитьСвойствоНаСервере(Свойство, Значение, ЭтоФорма, ИмяЭлемента);
	ЭтаФорма.ОбновитьОтображениеДанных();

КонецПроцедуры

&НаСервере
Функция ИзменитьСвойствоНаСервере(Свойство, Значение, ЭтоФорма, ИмяЭлемента)
	Результат = Ложь;
