...
Процедура НастройкиПроекта(Команда)
	...
	ОткрытьФорму("ВнешняяОбработка.Эскиз.Форма.НастройкиПроекта",, ЭтаФорма,,,, ОписаниеОповещенияОЗакрытии, РежимОткрытияОкнаФормы.БлокироватьОкноВладельца);
...
Процедура ОбработатьИзменениеНастроекПроекта() 
	Элементы.ГруппаКолонокРежимРедактированияСвойств.Видимость = Объект.ВключитьРежимРедактированияСвойств;
	Элементы.ГруппаКнопокРежимРедактированияСвойств.Видимость = Объект.ВключитьРежимРедактированияСвойств;
	
	Если Объект.ВключитьРежимРедактированияСвойств Тогда
...
Процедура ОпределитьСвойства(ТекущиеДанные) Экспорт
	...
	Для Каждого Строка Из ИскомыеСтроки Цикл
		...
		Если ТекущиеДанныеВыбора.Количество() Тогда
			ИскомыйЭлемент = ТекущиеДанныеВыбора.НайтиПоЗначению(Строка.Значение);
			Строка.Значение = ИскомыйЭлемент.Представление;
...
Процедура ДобавитьСвойство(СвойстваНаСервере, Свойство, Представление, Значение, ДанныеВыбора) 
	...
	НоваяСтрока.СвойствоПредставление = Представление;
...
Процедура СохранитьСвойстваВФайл(Команда)
	...
	//ОписаниеОповещенияОЗавершении = Новый ОписаниеОповещения("СохранитьСвойстваВФайлЗавершение", ЭтотОбъект);
	НачатьПолучениеФайлаССервера(, Результат.Адрес, Результат.ПутьКФайлу);
	
КонецПроцедуры

&НаКлиенте
Процедура СохранитьСвойстваВФайлЗавершение(ПолученныйФайл, ДополнительныеПараметры) Экспорт
...
Функция СохранитьСвойстваВФайлНаСервере()
	...
	СвойстваНаСервере = РеквизитФормыВЗначение("СвойстваФормИЭлементовУправления");
	Json = СериализоватьВJSON(ТаблицаЗначенийВМассивСтруктур(СвойстваНаСервере, Истина));
	
	Писатель = Новый ЗаписьТекста(ИмяВременногоФайла, КодировкаТекста.Системная);
...
Функция ТаблицаЗначенийВМассивСтруктур(Знач ТаблицаЗначений, ВсеКолонки, ИменаКолонок = "")
